...
public class Project054 {
	...
	public static void main(String[] args) {
		...
		int p2wins = 0;
		while(str.length() > 0) {
			String p1Hand = str.substring(0,14);
			if(str.length() > 14) {
				str = str.substring(15);
			}
			String p2Hand = str.substring(0,14);
			if(str.length() > 14) {
				str = str.substring(15);
			}
			int[] p1Eval = handEval(p1Hand);
			int[] p2Eval = handEval(p2Hand);
			
			if(p1Eval[0] > p2Eval[0]) {
				p1wins++;
			}
			else if(p1Eval[0] < p2Eval[0]) {
				p2wins++;
			}
			else {
				for(int i = 1; i < p1Eval.length; i++) {
					if(p1Eval[i] > p2Eval[i]) {
						p1wins++;
						break;
					}
					if(p1Eval[i] < p2Eval[i]) {
